...
async function activarPlan(req,res,next){try{
  ...
  const existente=db.prepare(`SELECT id FROM suscripciones WHERE usuario_id=? AND estado='activa' AND (periodo_hasta IS NULL OR datetime(periodo_hasta)>datetime('now')) ORDER BY creado_en DESC LIMIT 1`).get(req.usuario.id);
  if(existente)return res.status(409).json({error:'Ya tienes una suscripción activa.'});
  ...
  // Los intentos iniciales fallidos anteriores no deben bloquear una nueva activación.
  db.prepare(`UPDATE suscripciones SET estado='cancelada',cancelar_al_final=1,actualizado_en=datetime('now') WHERE usuario_id=? AND estado='morosa' AND periodo_desde IS NULL`).run(u.id);

  const now=new Date();const sid=nuevoId('sub');const fid=nuevoId('fps');
  const tx=db.transaction(()=>{
    ...
    db.prepare(`INSERT INTO suscripciones(id,usuario_id,plan_id,periodicidad,estado,prueba_inicio,prueba_hasta,periodo_desde,periodo_hasta,proximo_cobro,fuente_pago_id,max_ha_plan,creado_en,actualizado_en) VALUES(?,?,?,?,? ,NULL,NULL,NULL,NULL,NULL,?,?,datetime('now'),datetime('now'))`).run(sid,u.id,plan.id,periodicidad,'morosa',fid,plan.maxHa);
  });tx();

  const s=db.prepare('SELECT * FROM suscripciones WHERE id=?').get(sid);
  const {trx,monto}=await cobrarSuscripcion(s);
  const estado=String(trx.status||'PENDING').toUpperCase();

  if(estado==='APPROVED'){
    const desde=new Date();
    const finPagado=periodicidad==='anual'?addMonths(desde,12):addMonths(desde,1);
    const hasta=addDays(new Date(finPagado),7);
    db.prepare(`UPDATE suscripciones SET estado='activa',periodo_desde=?,periodo_hasta=?,proximo_cobro=?,intentos_fallidos=0,actualizado_en=datetime('now') WHERE id=?`).run(desde.toISOString(),hasta,hasta,sid);
    crearNotificacionUsuario({usuarioId:u.id,tipo:'suscripcion_activada',titulo:'Plan activado + 7 días gratis',mensaje:`Tu pago de $${monto.toLocaleString('es-CO')} COP fue aprobado. El plan ${plan.nombre} está activo e incluye 7 días adicionales gratis.`,entidadTipo:'suscripcion',entidadId:sid,prioridad:'alta'});
    crearNotificacionAdmin({tipo:'suscripcion_activada',titulo:'Nueva suscripción pagada',mensaje:`${u.nombre} activó el plan ${plan.nombre} por $${monto.toLocaleString('es-CO')} COP + 7 días gratis.`,usuarioId:u.id,entidadTipo:'suscripcion',entidadId:sid,prioridad:'alta'});
    audit({req,action:'activar_plan_suscripcion',entityType:'suscripcion',entityId:sid,metadata:{planId,periodicidad,ha,monto,estado}});
    return res.status(201).json({ok:true,suscripcionId:sid,estado,plan,periodicidad,monto,periodoDesde:desde.toISOString(),periodoHasta:hasta,proximoCobro:hasta,bonoDias:7});
  }

  // No se concede acceso hasta que Wompi confirme APPROVED. Un PENDING puede
  // resolverse después mediante webhook; los rechazados pueden intentarse de nuevo.
  db.prepare(`UPDATE suscripciones SET estado='morosa',proximo_cobro=NULL,actualizado_en=datetime('now') WHERE id=?`).run(sid);
  if(['DECLINED','ERROR','VOIDED'].includes(estado)){
    crearNotificacionUsuario({usuarioId:u.id,tipo:'suscripcion_fallo',titulo:'No pudimos activar tu plan',mensaje:'El primer cobro no fue aprobado. No se habilitó Mi Cultivo y puedes intentar nuevamente con otro medio de pago.',entidadTipo:'suscripcion',entidadId:sid});
  }
  audit({req,action:'activar_plan_suscripcion',entityType:'suscripcion',entityId:sid,result:estado==='PENDING'?'pendiente':'fallido',metadata:{planId,periodicidad,ha,monto,estado}});
  return res.status(estado==='PENDING'?202:402).json({ok:false,suscripcionId:sid,estado,plan,periodicidad,monto,error:estado==='PENDING'?'Tu pago está pendiente de confirmación. Mi Cultivo se habilitará cuando Wompi lo apruebe.':'El pago no fue aprobado. No se realizó la activación.'});
}catch(e){next(e)}}
router.post('/activar-plan',requiereAuth,activarPlan);
// Compatibilidad temporal con versiones anteriores de la PWA.
router.post('/activar-prueba',requiereAuth,activarPlan);
...
async function procesarRenovaciones(){
 ...
 for(const s of pendientes){try{
   ...
   if(estado==='APPROVED'){
     ...
     // Evita crear cobros duplicados cada 6 horas mientras Wompi confirma la transacción.
     db.prepare(`UPDATE suscripciones SET estado='morosa',proximo_cobro=?,actualizado_en=datetime('now') WHERE id=?`).run(addDays(new Date(),1),s.id);
